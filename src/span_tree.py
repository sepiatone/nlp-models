...
class Node:
    ...
    def merge_unary(self):
        ...
        #print('D', self.label, len(self.children),) 
        if len(self.children) == 1 and type(self.children[0]) != Token:
            #deleting the unary children!
            
            last_one = self.children[0]; self.label += '--' + last_one.label
            while (type(last_one) != Token and len(last_one.children) == 1 and type(last_one.children[0]) != Token):
                last_one = last_one.children[0]; self.label += '--' + last_one.label
            
            self.children = last_one.children
    ...
    def convert_binary(self):
        ...
        if len(self.children) > 2:
            right_child = SubTree()
            right_child.children = self.children[1:]
            right_child.parent = self
            for c in right_child.children:
                c.parent = right_child
            self.children = [self.children[0], right_child]
        for c in self.children:
            c.convert_binary()
    ...
    def __repr__(self) -> str:
        str_repr: str = ""
        if type(self) == Intent or type(self) == Slot or type(self) == SubTree:
...
class Tree:
    ...
    def merge_unary(self):
        for child in self.root.children: #root is usually unary
            child.merge_unary()

    def convert_binary(self):
        self.root.convert_binary()
        ...
        for item in top_repr.split():
            if item == BRACKET_CLOSE:
                ...
                if label.startswith(PREFIX_INTENT):
                    ...
                    node_stack.append(SubTree(label))
                else:
                    raise NameError(
